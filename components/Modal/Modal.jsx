        ...
        <div className={`${styles.modal__container}`} onClick={handleModalClick}>
          {/* The cart content */}
            ...
            <div className={styles.modal__header}>
              <button onClick={() => cart.setToggleModal(false)}><BsChevronRight /></button>
              <span>Cart</span>
            </div>
            ...
            <div className={styles.modal__content}>  
                {
                  cart.items.length <= 0 ?
                  <div className={styles.modal__content_empty}>Cart is Empty !</div> 
                  :
                  cart.items.map(cartItem => {
                    let item = getProductDataById(cartItem.id)
                    return <div className={styles.cartItem__container} key={cartItem.id}>
                      ...
                      <div className={styles.cartItem__info}>
                        ...
                        <div>
                          <h4 className={styles.cartItem__title}>{item.title.slice(0, 26)}</h4>
                          <p className={styles.cartItem__price}>${item.price}</p>

                          <div className={styles.cartItem__quantity__container}>
                            <button className={styles.cartItem__btn} onClick={() => cart.removeOneFromCart(cartItem.id)}>-</button>
                            <span>{cartItem.quantity}</span>
                            <button onClick={() => cart.addOneToCart(cartItem.id)}>+</button>
                          </div>
                        </div>
                 
                      </div>

                      <button className={styles.cartItem__delete} onClick={() => cart.deleteFromCart(cartItem.id)}><IoIosClose /></button>
                    </div>
                  })
                }
            </div>
            ...
            <div className={`${styles.modal__view_cart} btn`} onClick={checkout}>Checkout</div>
          </div>
